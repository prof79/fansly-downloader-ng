...
def download_media(config: FanslyConfig, state: DownloadState, accessible_media: list[MediaItem]):
    ...
    for media_item in accessible_media:
        ...
        if config.resolution != VideoResolution.NOTSET and not media_item.requested_resolution_found and media_item.mimetype == 'video/mp4':
            try:
                print_warning(f"Requested resolution {config.resolution.value}p ({config.resolution.description}) not found. Resolution set to {media_item.resolution.value}p ({media_item.resolution.description})\n")
            except AttributeError:
                print_warning(f"Requested resolution {config.resolution.value}p ({config.resolution.description}) not found. Resolution set to {media_item.resolution}p\n")

        # if show_downloads is True / downloads should be shown
        if config.show_downloads:
            if media_item.mimetype == 'video/mp4':
                try:
                    print_info(f"Downloading {media_item.mimetype.split('/')[-2]} '{filename}' in {media_item.resolution.value}p ({media_item.resolution.description})")
                except AttributeError:
                    print_info(f"Downloading {media_item.mimetype.split('/')[-2]} '{filename}' in {media_item.resolution}p")
            else:
